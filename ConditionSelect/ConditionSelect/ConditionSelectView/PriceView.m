...
@interface PriceView()<UITableViewDelegate,UITableViewDataSource>
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSNumber *number = [[NSUserDefaults standardUserDefaults] valueForKey:@"price_checkmark"];
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"价格%lu",indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [[NSUserDefaults standardUserDefaults] setValue:[[NSNumber alloc] initWithInteger:indexPath.row] forKey:@"price_checkmark"];
    [self.tableView reloadData];
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"priceCellNotification" object:nil];
    });
}
