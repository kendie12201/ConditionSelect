...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSNumber *number = [[NSUserDefaults standardUserDefaults] valueForKey:@"state_checkmark"];
    ...
    cell.textLabel.text = [NSString stringWithFormat:@"状态%lu",indexPath.row];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [[NSUserDefaults standardUserDefaults] setValue:[[NSNumber alloc] initWithInteger:indexPath.row] forKey:@"state_checkmark"];
    [self.tableView reloadData];
    
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.25 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [[NSNotificationCenter defaultCenter] postNotificationName:@"stateCellNotification" object:nil];
    });
}
